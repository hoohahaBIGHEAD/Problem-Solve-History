...
public:
    ...
    //링크드 리스트로 변경
    menuItemTypeNew* next = nullptr;
    menuItemTypeNew* prev = nullptr;

    menuItemTypeNew(string menuItem, double menuPrice) {
    ...
    //파일에서 메뉴를 읽어온다.
    void getDataNew(ifstream& inFile, menuItemTypeNew mList[], int listSize)
    {
        string name, str;
        double price;
    ...
    //메뉴를 보여준다.
    void showMenuNew(menuItemTypeNew mList[], int listSize)
    {
    ...
    void printCheckNew(menuItemTypeNew mList[], int listSize, int cList[], int cListLength)
    {
        double Total = 0;
        double Tax = 0;
    ...
    //메뉴가 기존에 선택되었는지 확인
    bool isItemSelectedNew(int cList[], int cListLength, int itemNo)
    {
    ...
    void makeSelectionNew(menuItemTypeNew mList[], int listSize, int choiceList[], int& cListLength)
    {
                        ...
                        if (ChooseMenu > 0 && ChooseMenu <= listSize)
                        {
                            ...
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    if (isItemSelectedNew(choiceList, cListLength, ChooseMenu))
                                    {
                                        //기존의 주문량에 추가한다.
